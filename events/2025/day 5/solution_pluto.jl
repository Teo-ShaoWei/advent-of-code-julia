...
# ╔═╡ 8a2c58e3-9f7d-4610-ba0b-dc8f601d2535
...
# ╔═╡ 774a5d0a-6ec1-4667-b01f-ea5fbfaea2e9
function parse_freshness(s)
    @chain s begin
        split("\n")
		@. parse_range
    end
...
# ╔═╡ 7bbe815f-0881-4015-8a70-94f9888af324
function parse_ingredients(s)
    @chain s begin
        split("\n")
		@. string
		@. parse(Int, _)
...
function parse_puzzle_data(s::String)
    @chain s begin
        split("\n\n")
		(
			freshness = parse_freshness(_[1]),
			ingredients = parse_ingredients(_[2]),
		)
    end
end
...
function result1((; freshness, ingredients))
	count(any(i ∈ f for f ∈ freshness) for i ∈ ingredients)
end
...
# ╔═╡ c5704375-17df-4c00-a142-006b9f9598e0
function merge_ranges(rs::Vector{UnitRange{Int}})
	sorted_rs = sort(rs; by = r -> r.start)
	
	merged_rs = [sorted_rs[1]]
	for r in sorted_rs[2:end]
		if !isempty(merged_rs[end] ∩ r)
			merged_rs[end] = range(
				start = merged_rs[end].start,
				stop = max(r.stop, merged_rs[end].stop),
			)
		else
			push!(merged_rs, r)
		end
...
function result2((; freshness))
	@chain freshness begin
		merge_ranges
		@. length
		sum
	end
