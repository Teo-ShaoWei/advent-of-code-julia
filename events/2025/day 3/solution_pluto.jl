...
# ╔═╡ ad19d77d-7715-4588-9a71-2a7e9245b180
function make_bank(n::BigInt)
	@chain n begin
		digits
		reverse!
		enumerate
...
# ╔═╡ 6ed9dd73-2cd0-4391-8284-bd2cd3c1d0ce
function select_next_batt(bank::Vector{Tuple{Int, Int}}; lower_bound::Int, upper_bound::Int)
	partialsort(
		bank[lower_bound:(end - upper_bound)],
		1,
		;
		by = ((index, joltage),) -> (-joltage, index),
	)
end

# ╔═╡ 62303755-9e9a-498c-b3ba-e94352c61132
function max_bank_joltage(n::BigInt; batt_usage_count::Int)
	bank = make_bank(n)
	
	selection = Int[]

	lower_bound = 1
	for i in 1:batt_usage_count
		(next_i, next_max_n) = select_next_batt(
			bank,
			;
			lower_bound,
			upper_bound = batt_usage_count - i,
		)
		push!(selection, next_max_n)
		lower_bound = next_i + 1
	end
	return parse(Int, join(selection))
end

# ╔═╡ 573f3f43-de61-4b32-995f-17df7b466e96
max_bank_joltage.(PDS_part1[1]; batt_usage_count = 2)

# ╔═╡ 4c4f78ee-b679-4597-9a0e-a01a2060f231
function result1(pd)
    @chain pd begin
        @. max_bank_joltage(_; batt_usage_count = 2)
		sum
    end
end
...
function result2(pd)
    @chain pd begin
        @. max_bank_joltage(_; batt_usage_count = 12)
		sum
    end
end
