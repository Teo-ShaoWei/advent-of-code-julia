...
# ╔═╡ c805d3f2-55a7-4dc7-841f-d66e02c69b26
function parse_ops(s)
	@chain s begin
		eachmatch(r"(\*|\+)", _)
		collect
		@. only
...
# ╔═╡ 36abc5f3-af06-470c-829c-270949d2f627
function split_empty_lines(m)
	groups = Matrix{Char}[]
	start_i = 1
	for i in axes(m, 1)
		if all(m[i, :] .== ' ')
			push!(groups, m[start_i:(i - 1), :])
			start_i = i + 1
		end
	end
	push!(groups, m[start_i:end, :])
	groups
end
...
solve_problem(; numbers::Vector, op) = eval(Meta.parse("$(op)($(join(numbers, ", ")))"))

# ╔═╡ 2005de4c-7c2d-4f85-a280-5fad7943ecfb
function solve_problem_part1(m::Matrix{Char})
	solve_problem(
		op = m[1, end],
		numbers = [
			@chain m[:, j] begin
...
# ╔═╡ 9584c66e-e856-4a01-9004-4e2112510873
solve_problem_part1(PDS_part1[1])
...
function solve_problem_part2(m::Matrix{Char})
	solve_problem(
		op = m[1, end],
		numbers = [
			@chain m[i, 1:(end - 1)] begin
				join
				parse(Int, _)
			end
			for i ∈ 1:size(m, 1)
...
# ╔═╡ f517204c-ecab-4d51-8a73-a6cb876738fe
solve_problem_part2(PDS_part2[end])
