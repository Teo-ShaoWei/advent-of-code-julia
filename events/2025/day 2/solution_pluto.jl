...
function is_repeated(d::Vector{Int}; num_splits)
	(split_size, leftover) = fldmod(length(d), num_splits)
	(leftover != 0) && return false
	split_range = (split_size + 1):split_size:length(d)
	unique_groups = Set(split(d, split_range...))
	return length(unique_groups) == 1
end

# ╔═╡ 81d582b5-fdbd-4e47-95e7-e20423cb34d3
[
	is_repeated([1, 4]; num_splits = 2),
	is_repeated([1, 1, 1]; num_splits = 2),
	is_repeated([1, 2, 3, 1, 2]; num_splits = 2),
]

# ╔═╡ 459873e5-8ab7-40de-b3f0-2de8c92c6012
[
	is_repeated([1, 1]; num_splits = 2),
	is_repeated([1, 2, 1, 2]; num_splits = 2),
	is_repeated([1, 3, 1, 3, 1, 3]; num_splits = 3),
	is_repeated([1, 2, 3, 1, 2, 3]; num_splits = 2),
]

# ╔═╡ e8f706b5-5e71-478f-839e-c25847c95372
function is_invalid(s::Int)
	@chain s begin
		digits
		is_repeated(_; num_splits = 2)
	end
end
...
function is_invalid_new(s::Int)
	@chain s begin
		digits
		(is_repeated(_; num_splits) for num_splits in 2:length(_))
		any
	end
end

# ╔═╡ 681b3171-ec6d-4e46-a7f1-0ac676c64ede
[
	is_invalid_new(121),
	is_invalid_new(13),
	is_invalid_new(12312),
]

# ╔═╡ d8a9099b-775f-4642-85ed-435cbac5e35e
[
	is_invalid_new(11),
	is_invalid_new(3333),
	is_invalid_new(331331331),
	is_invalid_new(1212),
]

# ╔═╡ e6938500-494f-4c3a-92e0-ba48fa360ec0
function result2(pd)
    sum(x for r ∈ pd for x ∈ r if is_invalid_new(x))
end
