...
function parse_puzzle_line(s)
    @chain s begin
		...
		split("")
		@. only
		[x == '.' ? 0 : -1 for x ∈ _]  # -1 if it's the starting point or a splitter, otherwise 0, which is empty.
    end
end
...
get_starting_col(area) = only(col for col ∈ axes(area, 2) if area[1, col] == -1)

# ╔═╡ 028a590a-3790-463a-b963-2c2f25ccb735
get_starting_col(PDS_part1)

# ╔═╡ 92dfb409-7e5d-4b02-8de2-066c65e217a6
function apply_starting_beam(area::Matrix)
	result_area = Matrix{Union{Char, Int}}(area)
	result_area[2, get_starting_col(area)] = 1
	return result_area
end

# ╔═╡ cd3531ed-eb22-4f6d-911d-f2fecbee2c79
function apply_next_beam(area, row)
	beam_cols = [col for col ∈ axes(area, 2) if area[row - 2, col] > 0]
	splitter_cols = [col for col ∈ axes(area, 2) if area[row - 1, col] == -1]

	result_area = copy(area)
	
	for col in axes(area, 2)
		if col ∈ beam_cols ∩ splitter_cols
			result_area[row - 1, col] = -2  # mark a splitter (-1) as used (-2)
			result_area[row, col - 1] += area[row - 2, col]
			result_area[row, col + 1] += area[row - 2, col]
		else
			result_area[row, col] += area[row - 2, col]
		end
	end
...
# ╔═╡ 3afe1a86-f245-447d-a4be-ccd1282f82b4
@chain PDS_part1 begin
	apply_starting_beam
	apply_next_beam(4)
	apply_next_beam(6)
	apply_next_beam(8)
...
function result1(pd)
    area = apply_starting_beam(pd)
	for row in 4:2:size(pd, 1)
		area = apply_next_beam(area, row)
	end
	return count(area .== -2)
end
